...
export function CompleteProfile(name, role) {
  ...
  if (name.trim() != "") {
    ...
    axios.defaults.headers.post['Token'] = localStorage.getItem('dt_auth_key');
    axios.defaults.headers.post['Authorization'] = "Bearer " + localStorage.getItem('jwt');
    return (dispatch) => (axios.post('http://localhost:7071/api/v1/profile', params, {withCredentials: true}).then(res => {
      dispatch({
        'payload': {
          nameStatus: null,
          error: false,
  ...
  if (email && password && confirmPassword) {
    if (password != confirmPassword) {
      ...
      return (dispatch) => {
        ...
        axios.defaults.headers.post['Authorization'] = "Bearer " + localStorage.getItem('jwt');
        axios
          .post('http://localhost:7071/api/v1/create-user', params, {withCredentials: true})
          .then(res => {
...
export function LoginToServer(email, password) {
  ...
  if (email && password) {
    return (dispatch) => {
      ...
      axios.defaults.headers.post['Authorization'] = "Bearer " + localStorage.getItem('jwt');
      axios
        .post('http://localhost:7071/api/v1/validate-user', params, {withCredentials: true})
        .then(res => {
          switch (res.status) {
            case 200:
              if (res.data.status == 1) {
